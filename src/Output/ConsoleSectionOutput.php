...
namespace PhacMan\ConsoleTable\Output;

use function array_slice;
use function count;
use PhacMan\ConsoleTable\Formatter\OutputFormatterInterface;
...
class ConsoleSectionOutput extends StreamOutput
{
    ...
    public function setMaxHeight(int $maxHeight) : void
    {
        ...
        StreamOutput::doWrite($this->getVisibleContent(), false);
        StreamOutput::doWrite($existingContent, false);
    }
    ...
    public function clear(int $lines = null) : void
    {
        ...
        StreamOutput::doWrite($this->popStreamContentUntilCurrentSection($this->maxHeight ? min($this->maxHeight, $lines) : $lines), false);
    }
    ...
    protected function doWrite(string $message, bool $newline) : void
    {
        ...
        if (!$this->isDecorated()) {
            StreamOutput::doWrite($message, $newline);
        ...
        if ($lineOverflow) {
            ...
            StreamOutput::doWrite(implode('', $previousLinesOfSection), false);
        }

        // if the last line was removed, re-print its content together with the new content.
        // otherwise, just print the new content.
        StreamOutput::doWrite($deleteLastLine ? $lastLine . $message : $message, true);
        StreamOutput::doWrite($erasedContent, false);
    }
    ...
    private function popStreamContentUntilCurrentSection(int $numberOfLinesToClearFromCurrentSection = 0) : string
    {
        ...
        if ($numberOfLinesToClear > 0) {
            ...
            StreamOutput::doWrite(sprintf("\x1b[%dA", $numberOfLinesToClear), false);
            // erase to end of screen
            StreamOutput::doWrite("\x1b[0J", false);
        }
