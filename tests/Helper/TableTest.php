...
use PhacMan\ConsoleTable\Exception\RuntimeException;
...
use PhacMan\ConsoleTable\Output\ConsoleSectionOutput;
use PhacMan\ConsoleTable\Output\StreamOutput;
...
TABLE;
    ...
    public function no_testSectionOutput() // TODO strange
    {
        $sections = [];
...
TABLE;
    ...
    public function no_testSectionOutputDoesntClearIfTableIsntRendered() // TODO strange
    {
        $sections = [];
        $stream = $this->getOutputStream(true);
        $output = new ConsoleSectionOutput($stream->getStream(), $sections, $stream->getVerbosity(), $stream->isDecorated(), new OutputFormatter());
...
TABLE;
    ...
    public function no_testSectionOutputWithoutDecoration() // TODO strange
    {
        $sections = [];
...
TABLE;
    ...
    public function no_testAppendRowWithoutSectionOutput() // TODO strange
    {
        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('Output should be an instance of "PhacMan\ConsoleTable\Output\ConsoleSectionOutput" when calling "PhacMan\ConsoleTable\Helper\Table::appendRow".');
        $table = new Table($this->getOutputStream());

        $table->appendRow(['9971-5-0210-0', 'A Tale of Two Cities', 'Charles Dickens', '139.25']);
    }

    public function no_testSectionOutputHandlesZeroRowsAfterRender() // TODO strange
    {
        $sections = [];
        $stream = $this->getOutputStream(true);
        $output = new ConsoleSectionOutput($stream->getStream(), $sections, $stream->getVerbosity(), $stream->isDecorated(), new OutputFormatter());
...
EOTXT
    ...
    public function no_testWithHyperlinkAndMaxWidth() // TODO strange
    {
